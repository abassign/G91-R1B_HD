...
var pilot_assistant = func() {
    ...
    if (pilot_ass_status_id == 1 and pilot_ass_status_id < 10 and airport_select == nil) {
        ...
        var circuit = getprop("fdm/jsbsim/systems/autopilot/gui/landing-circuit-L-R");
        holding_point.apply_course_distance(airport_select.runways[rwy_select].heading + 90.0 * circuit,3.0 * 1852.0); #// With +90 you should have a right-hand circuit -90 for the left-hand circuit
        ...
        if (pilot_ass_status_id == 2.0) {
            ...
            if (landing_20_dist_to_reduce_v == 1) {
                ...
                if ((getprop("fdm/jsbsim/systems/autopilot/pitch-angle-absolute-deg-lag") < -6.0) or (math.abs(getprop("fdm/jsbsim/systems/autopilot/true-heading-turn-direction-versus-slope-sw-lag-deg")) > 20.0)) {
...
#//print("***** ",landing_20_dist_to_reduce_h," | ",airplane.alt()," | ",runway_alt_m_select," | ",holding_point_h_ft," | ",holding_point_to_airplane_delta_alt_ft," | ",holding_point_to_airplane_dist_nm," | ",dist_to_reduce_h);
            if (holding_point_to_airplane_delta_alt_ft < 0.0 and landing_20_dist_to_reduce_h == 1) {
                ...
                setprop("fdm/jsbsim/systems/autopilot/gui/vertical-speed-fpm",3500);
            ...
            if (holding_point_to_airplane_dist_nm > holding_point_before_distance_nm and holding_point_before_distance_nm > 0.0) {
                ...
                if (testing_log_active >= 1 and timeStepSecond == 1) {
                    ...
                    ,sprintf(" Hoding dist: %5.1f",holding_point_to_airplane_dist_nm_direct)
                    ...
                    ,sprintf(" | H bef: %4.1f",heading_correction_deg_for_before_dist)
                    ,sprintf(" Hld. pt to plane: %4.1f",holding_point_to_airplane_dist_nm)
                    ,sprintf(" Hld. pt: %3.1f",holding_point_before_distance_nm)
                    ,sprintf(" Dis R: %3.1f",dist_to_reduce_h)
                    ,sprintf(" Dist red: %5.0f",landing_20_dist_to_reduce_h_fpm)
                    ,sprintf(" CAS: %3.0f",speed_cas)
                    ,sprintf(" | %3.0f",landing_20_dist_to_reduce_v_cas)
                    ...
                    ,sprintf(" Impact ctl: %2.1f",impact_ctl_attention)
                    );
                }
            } else {
                ...
                setprop("fdm/jsbsim/systems/gauges/PHI/program/reset",2); #// For PHI reset
            ...
            if (math.abs(rwy_point_delta_alt_ft) < 1000.0 and isHolding_reducing_terminated == 0) {
                ...
                if (impact_ctl_attention == 0) {
                    if ((rwy_point_delta_alt_ft > 0.0) and (math.abs(getprop("fdm/jsbsim/systems/autopilot/true-heading-turn-direction-versus-slope-sw-lag-deg")) < 20.0)) {
                        setprop("fdm/jsbsim/systems/autopilot/speed-brake-set-activate",1.0);
            ...
            if ((math.abs(getprop("fdm/jsbsim/systems/autopilot/speed-cas-lag") - 250.0)) < 40.0 and math.abs(rwy_point_delta_alt_ft) < 500.0 and isHolding_reducing_terminated == 1) {
    ...
    if (pilot_ass_status_id >= 10.0 and pilot_ass_status_id < 11.0) {
        ...
        if (pilot_ass_status_id == 10.0) {
            ...
            var altitude_agl_ft = getprop("/position/altitude-agl-ft");
            var altitude_agl_ft_ass = getprop("fdm/jsbsim/systems/autopilot/h-sl-ft-lag") - runway_alt_m_select;
            if ((altitude_agl_ft > 10.0) and (altitude_agl_ft_ass > altitude_agl_ft)) {
                altitude_agl_ft = altitude_agl_ft_ass;
            }
            ...
            print("Automatic take-Off procedure end");
...
setlistener("fdm/jsbsim/systems/autopilot/gui/landing-circuit-L-R", func {

    var circuit = getprop("fdm/jsbsim/systems/autopilot/gui/landing-circuit-L-R");
    print("***** set Circuit: ",circuit);
    if (circuit > 0.5) {
        setprop("fdm/jsbsim/systems/autopilot/gui/landing-circuit-L-R",1.0);
        setprop("fdm/jsbsim/systems/autopilot/gui/landing-circuit-L-R-description","Right");
    } else if (circuit < -0.5) {
        setprop("fdm/jsbsim/systems/autopilot/gui/landing-circuit-L-R",-1.0);
        setprop("fdm/jsbsim/systems/autopilot/gui/landing-circuit-L-R-description","Left");
    } else {
        setprop("fdm/jsbsim/systems/autopilot/gui/landing-circuit-L-R",0.0);
        setprop("fdm/jsbsim/systems/autopilot/gui/landing-circuit-L-R-description","Central");
    };
