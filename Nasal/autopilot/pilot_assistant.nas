...
var RunwayDataClass = {
    ...
    selectorByHeading: func(landing_airport_search_max_heading = 0.0) {
        ...
        if (math.abs(geo.normdeg(me.heading_true_deg - airplane.course_to(me.coord))) <= landing_airport_search_max_heading) {
    ...
    selectorByWind: func(landing_max_lateral_wind = 0.0) {
        ...
        var wind_deviation = math.abs(geo.normdeg(wind_from - me.heading));
...
var RunwaysDataClass = {
    ...
    init: func(airport) {
        ...
        # print("pilot_assistan.nas RunwayDataClass.init num_rwys: ",me.num_rwys," rwy: ",debug.dump(me.rwys));
    },
...
var AirportDataClass = {
    ...
    selectorByHeading: func(landing_airport_search_max_heading = 0.0) {
        if (landing_airport_search_max_heading == 0.0) landing_airport_search_max_heading = me.landing_airport_search_max_heading;
        var heading_true_deg = getprop("fdm/jsbsim/systems/autopilot/heading-true-deg");
        if (math.abs(geo.normdeg(heading_true_deg - airplane.course_to(me.apt_coord))) <= landing_airport_search_max_heading) {
            me.select_by_heading = 1;
        } else {
            me.select_by_heading = 0;
        }
...
var AirportsDataClass = {
    ...
    init: func(select_nearest = 0, select_by_heading = 0, distance_max = 0.0, airport_name = nil) {
        var airports = {};
        if (airport_name == nil) {
            if (distance_max == 0.0) distance_max = getprop("fdm/jsbsim/systems/autopilot/gui/landing-rwy-search-distance-max");
            me.distance_max = distance_max;
            var airplane = geo.aircraft_position();
            var near_airports = findAirportsWithinRange(airplane, me.distance_max);
            ...
            if (size(airports) == 0) {
                if (distance_max == 0.0) distance_max = getprop("fdm/jsbsim/systems/autopilot/gui/airport_select_id_direct_max_distance");
                me.distance_max = distance_max;
                var airplane = geo.aircraft_position();
                var airportsToSearch = findAirportsWithinRange(airplane, me.distance_max);
                print("***** ",me.distance_max," ",size(airportsToSearch));
                var num_airport = 0;
        ...
        foreach(var key_airport; keys(airports)) {
            print("pilot_assistan.nas AirportsDataClass.init key_airport: ",key_airport);
            airport = airports[key_airport];
            me.num_airports += 1;
        ...
        print("pilot_assistan.nas AirportsDataClass.init num_airport: ",me.num_airports," airports: ",debug.dump(me.airports));
    },
    
    nearestAirport: func() {
        var airportSelected = nil;
        foreach(var key_airport; keys(me.airports)) {
            airportSelected = me.airports[key_airport];
        };
...
var runway_finder = func(airport, all_direction, landing_rwy_search_distance_max) {

    #// Select the airport nearest in the frontal direction or direct
    apt_coord.set_latlon(airport.lat,airport.lon);
    var airplane = geo.aircraft_position();
    var heading_true_deg = getprop("fdm/jsbsim/systems/autopilot/heading-true-deg");
    var landing_airport_search_max_heading = getprop("fdm/jsbsim/systems/autopilot/gui/landing-airport-search-max-heading");
    ...
    if (landing_rwy_search_distance_max > 0.0) distance_to_airport_min = landing_rwy_search_distance_max;
    ...
    if (airport_heading <= landing_airport_search_max_heading
        ...
        foreach(var rwy; keys(airport.runways)) {
            if (airport.runways[rwy].length >= landing_minimal_length_m) {
                ...
                if (wind_compatibility_ok == 1) {
                    rwy_coord.set_latlon(airport.runways[rwy].lat,airport.runways[rwy].lon);
                    var runway_to_airplane_dist_nm = runway_to_airplane_dist(airport,rwy);
                    ...
                    print("pilot_assistant.nas runway_finder - found airport: ",airport.id," rwy not wind_compatibility: ",rwy);
                }
            } else {
                print("pilot_assistant.nas runway_finder - found airport: ",airport.id," rwy too short : ",rwy);
            }
        }
    }
    return nil;
...
var airport_rwy_finder = func(airport_name, all_direction, scan_mode) {
    ...
    if (size(airport_name) > 4) {
        #// Found the airport by name
        ## var airport_select_id_direct_node = props.globals.getNode("/fdm/jsbsim/systems/autopilot/gui/airport_select_id_direct", 1);
        
    } elsif (size(airport_name) == 4) {
        #// Found the airport by id 4 char
        ## airport = airportinfo(airport_name);
    }
    
    var alternative = AirportsDataClass.new();
    alternative.init(0,0,0.0,airport_name);
    if (alternative.num_airports > 0) {
        airport = alternative.nearestAirport().airport;
    }
    
    if (airport != nil) {
        if (scan_mode == 0) {
            rwy = runway_finder(airport, all_direction, 0.0);
        } else {
            rwy = runway_finder(airport, all_direction, getprop("fdm/jsbsim/systems/autopilot/gui/landing-rwy-search-distance-max"));
        }
        if (rwy != nil) {
            ...
            print("pilot_assistant.nas airport_rwy_finder - found airport: ",airport.id," name: ",airport_name," distance: ",distance);
            if (rwy != nil) {
                if (distance <= maxDistance) {
                    ...
                    print("pilot_assistant.nas airport_rwy_finder - found airport: ",airport.id," rwy: ",rwy);
                } else {
                    rwy = nil;
            ...
            print("pilot_assistant.nas airport_rwy_finder - no rwy found for airport: ",airport.id);
        } 
    }
    return rwy;
}
...
setlistener("fdm/jsbsim/systems/autopilot/gui/airport_select_id_direct/status", func {
    if (getprop("fdm/jsbsim/systems/autopilot/gui/airport_select_id_direct/status") == 1) {
        ...
        var rwy = airport_rwy_finder(airport_name,1,0);
        if (rwy != nil) {
            pilot_ass_status_id = 1.0;
        } else {
            setprop("fdm/jsbsim/systems/autopilot/gui/airport_landing_status","");
