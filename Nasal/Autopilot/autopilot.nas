...
var landing_slope_integral = 0.0;
...
var impact_altitude_hold_inserted_delay = 0;
...
var airport_searcher = maketimer(0.5, func() {
    ...
    if (landig_status_id == 1 and airport_select == nil) {
        ...
        if (landig_status_id == 2.0) {
            ...
            if ((airplane.distance_to(rwy_coord_start) * 0.000621371) > 0.4) {
                if (runway_to_airplane_dist_direct <= 12.0 and runway_to_airplane_dist_direct >= 5.0) {
                    setprop("fdm/jsbsim/systems/autopilot/gui/speed-value",180.0);
                    setprop("fdm/jsbsim/systems/autopilot/speed-brake-set-active",1.0);
                    setprop("fdm/jsbsim/systems/autopilot/gui/speed-automatic-flap",1.0);
                    setprop("fdm/jsbsim/systems/autopilot/gui/impact-control-active",2.0);
                }
                if (runway_to_airplane_dist_direct < 6.0) {
                ...
                ,sprintf(" Slope int: %.2f",landing_slope_integral)
    ...
    if (impact_control_active > 0.0 and (getprop("velocities/speed-east-fps") != 0 or getprop("velocities/speed-north-fps") != 0)) {
        ...
        var impact_ramp_max = 24.0 * (speed_cas / 250.0);
        ...
        if (impact_control_active > 0) {
            if ((impact_time < 8.0 and impact_is_in_security_zone == 0)
            and (((impact_time / impact_altitude_grain) < 0.6 and (impact_altitude_direction_der_frist > 2.0)) 
            ...
            or ((abs(impact_time_dif_0_10) < impact_altitude_grain) and (impact_altitude_direction_der_frist > 2.0)))) {
                ...
                if (impact_vertical_speed_max_fpm >= 0) {
                    ...
                    impact_altitude_hold_inserted_delay = 0;
                ...
                if (impact_is_in_security_zone) {
                    if (impact_altitude_hold_inserted_delay > 5) {
                ...
                if (impact_altitude_hold == 0) {
                    if (altitude_delta > impact_min_z_ft) {
                        ...
                        alpha = (altitude_delta - impact_min_z_ft) / speed_fps;
                        impact_ramp_delta = alpha * 10 / impact_medium_time_gui;
