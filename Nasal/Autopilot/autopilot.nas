...
var pilot_assistant = func {
    ...
    if (landig_departure_status_id == 1 and landig_departure_status_id < 10 and airport_select == nil) {
        ...
        if (landig_departure_status_id == 2.0) {
            ...
            if (((airplane.distance_to(rwy_coord_start) * 0.000621371) < 1.0 and altitude_agl_ft < 300.0) == 0) {
                ...
                landing_slope_integrate = landing_slope_integrate + error * timeStep;
                var derivate = (error - landing_slope_previous_error) / timeStep;
                var landing_slope = kp * error + ki * landing_slope_integrate + kd * derivate;
                landing_slope_previous_error = error;
                # cut funtion
                if (landing_slope > 6.0) {
                    landing_slope = 6.0;
                    landing_slope_integrate = 0.0;
                } else if (landing_slope < -6.0) {
                    landing_slope = -6.0;
                    landing_slope_integrate = 0.0;
                }
                ...
                ,sprintf(" Slope: %.2f",landing_slope)
                ,sprintf(" (%.2f|",landing_slope_target_integrate)
                ,sprintf("%.2f|",slope)
                ,sprintf("%.2f|",error)
    ...
    if (landig_departure_status_id >= 10) {
        ...
        if (landig_departure_status_id == 10.0) {
            ...
            print("Departure 10.0 >",
                " Airport: ",airport.id,
                " ",airport.runways[rwy_select].id,
                " ",airport.runways[rwy_select].length);
            if (airport_select != nil) {
    ...
    if (impact_control_active > 0.0 and (getprop("velocities/speed-east-fps") != 0 or getprop("velocities/speed-north-fps") != 0)) {
        ...
        if (impact_control_active > 0 and geod != nil) {
            if ((impact_allarm_solution_delay > 0) or (((impact_time < 5.0) and (impact_time_dif_0_10 > -0.25) and (impact_is_in_security_zone == 0 or (impact_is_in_security_zone == 1 and impact_time_dif_0_10 < 1.0)))
            and ((((impact_time / impact_altitude_grain) < 1.2) and math.abs(impact_altitude_direction_der_frist) > 1)
            ...
            or ((abs(impact_time_dif_0_10) < impact_altitude_grain) and (impact_altitude_direction_der_frist > 2.0))))) {
                ...
                if (impact_time_delta > 0.05) {
                    impact_ramp = impact_ramp + 1.5 * impact_ramp_delta / impact_medium_time_gui;
                } else if (impact_time_delta < -0.02) {
                    impact_ramp = impact_ramp - 1.0 * impact_ramp_delta / impact_medium_time_gui;
                }
